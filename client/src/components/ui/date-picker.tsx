    ...
    <div className="w-full mt-1.5">
      {label && (
        <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 mb-3  block">
          {label.includes('*') ? (
            <>
              {label.replace(' *', '')} <span className="text-muted-foreground">*</span>
            </>
          ) : (
            label
          )}
          ...
          customInput={
            <input
              className={cn(
                ...
                error && "border-foreground border-[1.5px] focus-visible:ring-ring",
                className
              )}
      ...
      {error && (
        <p className="text-sm text-foreground font-medium mt-1">{error}</p>
      )}
    </div>
  );
}
